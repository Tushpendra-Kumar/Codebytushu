...
| `/api/admin/` | Admin AJAX APIs: blogs, courses, leetcode, users, dashboard, settings, payments, messages, import_pdf |
...
| `/admin/` | Full Admin Panel (dashboard, blogs, courses, leetcode, users, analytics, uploads, settings, payments) |
| `/admin/includes/` | Admin-specific includes: `auth_check.php`, `header.php`, `sidebar.php`, `footer.php`, `pagination.php` |
| `/user/` | User-facing protected pages: `dashboard.php`, `purchases.php`, `courses.php`, `settings.php`, `orders.php`, `certificates.php`, `downloads.php` |
| `/blogs/` | Blogs module: `index.php` (DB-driven listing), `blog-details/` |
...
| `/video-editing/` | Video Editing module: `index.php` |
| `/store/` | Store module: `index.php`, `product-details/`, `cart/` |
| `/checkout/` | Checkout pages: `index.php`, `success.php` |
| `/cart/` | Cart pages (legacy HTML + PHP) |
| `/database/` | SQL schemas (`schema.sql`) and migrations (`migrations/001–012`) |
| `/database/backups/` | Database backup files |
| `/private/` | Server-only files: `courses/` (PDFs), `logs/` (error logs), migration tools |
| `/uploads/` | User-uploaded content (gitignored) |
| `/docs/` | All developer/AI documentation |
...
| Module | Backend | Auth-Gated | Admin CMS |
|---|---|---|---|
| Landing Page | Static HTML | ❌ Public | N/A |
| Blogs | ✅ DB-powered | ✅ Login Required | ✅ Full CMS |
| Courses | ✅ DB-powered + Purchase | ✅ Login Required | ✅ Full CMS |
| LeetCode | ✅ DB-powered | ✅ Login Required | ✅ Full CMS |
| Video Editing | ✅ DB-powered | ✅ Login Required | Partial |
| Store | ❌ Frontend only | ❌ Public | ❌ Pending |
| Auth System | ✅ Full flow | N/A | N/A |
| Admin Panel | ✅ Full CMS | ✅ Admin Only | N/A |
| User Dashboard | ✅ DB-powered | ✅ Login Required | N/A |
...
4. **Firebase is LEGACY**: `firebase-config.js` exists in root but is NOT the active auth system. PHP Sessions + Google OAuth via `api/auth/` is the live system.
