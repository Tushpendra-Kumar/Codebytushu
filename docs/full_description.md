...
- **Email**: PHPMailer (SMTP) via `classes/Mailer.php` for transactional emails (verification, password reset, shipping confirmations).
- **Environment**: `vlucas/phpdotenv` for secure `.env` loading via Composer.
- **Print-on-Demand**: Qikink API integration for merchandise fulfillment (webhook-based status updates).
...
| `/api/admin/` | Admin AJAX APIs: blogs, courses, leetcode, users, dashboard, settings, payments, messages, categories, import_pdf, store-products, store-orders |
| `/api/cart/` | Course cart APIs: `add.php`, `remove.php`, `view.php` |
| `/api/checkout/` | Course checkout APIs: `submit.php`, `submit_single.php` |
| `/api/courses/` | Course download API: `download.php` (secure PDF streaming) |
| `/api/store/` | Store APIs: `checkout.php` (store order creation), `invoice.php` (HTML invoice generator) |
| `/api/webhooks/` | Webhook receivers: `qikink.php` (Qikink fulfillment status updates) |
...
| `/admin/` | Full Admin Panel (dashboard, blogs, courses, leetcode, users, analytics, uploads, settings, payments, messages, newsletter, categories, store-products, store-orders) |
...
| `/store/` | Store module: `index.php` (DB-driven listing), `store.css`, `js/` |
| `/store/js/` | Store JS: `data.js` (legacy static data), `store.js` (cart logic), `toast.js` (notification UI) |
| `/store/product-details/` | Static product detail page (`index.html`) — pending PHP migration |
| `/store/cart/` | Static store cart page (`index.html`) — pending PHP migration |
| `/store/checkout/` | Store checkout: `index.php` (checkout form, login-required), `success.php` (order success) |
| `/store/order-tracking/` | Order tracking: `index.php` |
| `/checkout/` | Course checkout pages: `index.php`, `success.php` |
| `/cart/` | Course cart page: `index.php` (PHP, active) |
| `/database/` | SQL schemas (`schema.sql`) and migrations (`migrations/002–013`, 15 files) |
| `/private/` | Server-only files: `courses/` (PDFs), `logs/` (error + webhook logs) |
...
## 5. Key Module States (August 2026)
...
| Store Listing | ✅ DB-powered | ❌ Public | ✅ Full CMS | `/store/index.php` |
| Store Checkout | ✅ DB-powered | ✅ Login Required | ✅ Order Mgmt | `/store/checkout/index.php` + `/api/store/checkout.php` |
| Store Product Detail | ❌ Static HTML | ❌ Public | N/A | Legacy `store/product-details/index.html` — pending PHP migration |
| Store Cart | ❌ localStorage | ❌ Public | N/A | `store/cart/index.html` — pending DB migration |
...
| Cart/Checkout (Courses) | ✅ DB-powered | ✅ Login Required | N/A | `/cart/index.php` + `/checkout/*.php` |
| Qikink Webhooks | ✅ Receiver live | N/A | N/A | `/api/webhooks/qikink.php` |
...
7. **Store/Course Separation**: Course purchasing uses `/api/cart/` + `/api/checkout/`. Store purchasing uses `/api/store/checkout.php`. Orders are differentiated by `order_type` field (`course` vs `store`).
