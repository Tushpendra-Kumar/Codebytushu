...
| `GET /api/cart/view.php` | Get cart contents |
| `POST /api/checkout/submit.php` | Submit checkout (multiple items) |
| `POST /api/checkout/submit_single.php` | Submit single-item checkout |
...
| `POST /api/upload.php` | File upload handler (admin) |
| `GET/POST /api/courses/download.php` | Secure course PDF download |
...
| `GET /api/admin/payments.php` | Payment/order data |
| `GET /api/admin/messages.php` | Contact messages inbox |
| `GET /api/admin/categories.php` | Category management |
...
- **Analytics Tracker**: `includes/analytics.php` — Logs page views, device type, referrer.
- **Admin Sidebar/Header**: `admin/includes/sidebar.php`, `admin/includes/header.php` — Admin panel navigation.
- **User Sidebar/Header**: `user/includes/sidebar.php`, `user/includes/header.php` — User dashboard navigation.

## 7. Security & Connections Audited
...
- **Rate Limiting**: Contact form and login attempts are rate-limited (DB-backed).
