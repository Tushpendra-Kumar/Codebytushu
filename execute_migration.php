...
try {
    foreach ($files as $filePath) {
        ...
        // Year detection
        $ys = $pdo->prepare('SELECT id FROM leetcode_years WHERE year=? LIMIT 1');
        $ys->execute([$year]);
        $yr = $ys->fetch();
        $yearId = $yr ? (int)$yr['id'] : null;

        if (!$yearId) {
            $pdo->prepare('INSERT IGNORE INTO leetcode_years (year, total_months, status) VALUES (?, 12, "active")')->execute([$year]);
            $yearId = (int)$pdo->lastInsertId();
            if (!$yearId) {
                $ys->execute([$year]);
                $yearId = (int)($ys->fetch()['id']);
            }
        }

        // Month detection
        $ms = $pdo->prepare('SELECT id FROM leetcode_months WHERE year_id=? AND month_num=? LIMIT 1');
        $ms->execute([$yearId, $month]);
        $mon = $ms->fetch();
        ...
        $stmt = $pdo->prepare('INSERT INTO leetcode_solutions 
            (month_id, solution_date, day_number, year, month, slug, problem_title, problem_number, difficulty, time_complexity, space_complexity, youtube_url, is_published, published_at, created_at, updated_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())');
        
        $stmt->execute([
            $monthId, $date, $day, $year, $month, $slug, $problemTitle, $problemNumber, 'Medium', $timeComplexity, $spaceComplexity, $videoUrl, $isPublished, $publishedAt
        ]);
...
echo json_encode([
    ...
    'records_skipped_duplicate' => $skippedRecords,
    'errors' => $errors
], JSON_PRETTY_PRINT);
